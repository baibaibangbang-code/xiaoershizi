function buildPrompt({ theme, title, wordBank }) {
  const rolesList = wordBank.roles.map(w => `${w.pinyin} ${w.hanzi}`).join(', ');
  const itemsList = wordBank.items.map(w => `${w.pinyin} ${w.hanzi}`).join(', ');
  const envList = wordBank.environment.map(w => `${w.pinyin} ${w.hanzi}`).join(', ');
